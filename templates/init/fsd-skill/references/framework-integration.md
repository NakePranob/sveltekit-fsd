...
SvelteKit is the one framework where routing moves **into** the FSD app layer
instead of sitting beside it. `kit.files` makes every framework directory
configurable, so there is no root-level `routes/` left to reconcile — which also
means this is the only integration here where the framework's own config, not a
convention, is what enforces the structure.
...
my-sveltekit-project/
  src/
    app/                   ← FSD app layer
      index.html           ← appTemplate (moved from src/app.html)
      routes/              ← SvelteKit routing, now a segment of the app layer
        +layout.svelte     ← composition root: stylesheet, providers
        +page.svelte       ← renders an FSD page, nothing else
        login/+page.svelte
    ...
    pages/                 ← FSD pages layer
      home/
        ...
        index.ts
      login/
        ui/login-page.svelte
        ui/login-form.svelte
        index.ts
    shared/                ← FSD shared layer
      ui/
      lib/
...
Recent versions of `sv create` put the SvelteKit config inside the Vite config
and write no `svelte.config.js` at all; older projects have the opposite shape.
SvelteKit reads the Vite config **first** and only falls back to
`svelte.config.js`, so put this wherever the project already configures the
`kit`/`sveltekit()` options — writing it to the file that is not being read is a
silent failure, with correct values that nothing loads.
...
```svelte
<!-- src/app/routes/login/+page.svelte: thin route entry -->
<script lang="ts">
...
There is nothing to forward but the component: a Svelte page carries its own
`<svelte:head>`, so the title travels with it rather than being re-exported the
way a Next.js `metadata` object has to be.

A `+page.ts`, `+page.server.ts` or `+layout.server.ts` beside the route file is
the framework's, not FSD's. Keep it thin the same way: `load` fetches, and the
types and transformations it returns belong to a slice.

### Runes and file naming

A module that uses `$state` or `$derived` must be named `*.svelte.ts` — the
compiler only processes runes in `.svelte` components and in modules named that
way, and a rune in a plain `.ts` file becomes an undefined function at runtime
with nothing failing at build time. Inside a slice that lands in the `model`
segment as `model/<name>.svelte.ts`, imported without the `.ts`:

```typescript
import { useCart } from './model/cart.svelte';
```

Export a function that returns getters rather than the `$state` variable itself;
exporting the variable exports the value it held at module load.

### Steiger

`src/app/routes/**` belongs in Steiger's `ignores`. It is a segment of the app
layer whose every file is named `+page.svelte`, so the linter would read that
tree as slices and report on names SvelteKit chose. So do `src/hooks.*.ts`,
`src/params/`, `src/service-worker.ts` and `src/app.d.ts` — framework entry
points that live at the top of `src/` by definition. The import boundary inside
the route tree is still worth enforcing; an ESLint `no-restricted-imports` block
is what can see an import at all.

## Key Reminders for All Frameworks

1. **FSD lives in `src/`**: root-level `app/` and `pages/` belong to the
   framework's routing, not FSD. SvelteKit is the exception — `kit.files` makes
   the routing directory configurable, so it moves inside the FSD app layer
...
3. **Path aliases are required**: configure both the bundler and
   `tsconfig.json` — except in SvelteKit, which generates the tsconfig half from
   `kit.alias` for you.
